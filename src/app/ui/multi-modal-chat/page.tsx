...
export default function MultiModalChatPage() {
    ...
    // keep the latest attachments available to the unmount cleanup below
    useEffect(() => {
        attachmentsRef.current = attachments
    })

    // revoke any leftover preview object URLs when the component unmounts
    useEffect(() => () => {
        attachmentsRef.current.forEach((a) => a.preview && URL.revokeObjectURL(a.preview))
    }, [])

    // grow the textarea with its content, capped at a comfortable max height
    const autoResize = () => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (!canSend) return
        // rebuild a FileList from the attachments for the API
    ...
    const handleFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // replace the current selection and revoke its previews
        attachments.forEach((a) => a.preview && URL.revokeObjectURL(a.preview))
        setAttachments(
        ...
        // reset the input value so picking the same file again re-triggers onChange
        e.target.value = ""
    }
                                        ...
                                        switch (part.type) {
                                            ...
                                            case "file":
                                                return part.mediaType.startsWith("image/") || part.mediaType === "image" ? (
                                                    <img src={part.url} alt={part.filename ?? "attachment"} className="max-h-56 w-auto rounded-lg border border-zinc-700" key={`${message.id}-${index}`} />
                                                ) : (
                        ...
                        {attachments.map(({ file, preview }, index) => (
                            preview ? (
                                <div key={`${file.name}-${index}`} className="relative" title={file.name}>
                                    <img src={preview} alt={file.name} className="h-16 w-16 rounded-lg border border-zinc-700 object-cover" />
                                    <button
                                        type="button"
